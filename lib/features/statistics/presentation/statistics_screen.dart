      ...
      child: ListView(
        ...
        children: [
          const AppSectionTitle(
            ...
            subtitle: 'Länder, Städte, Reisetage und Erinnerungen aus deinen echten Reisen.',
          ),
          Padding(
...
class _HeroStatsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      children: [
        _StatTile(icon: Icons.flight_takeoff_rounded, label: 'Reisen', value: insights.tripCount.toString()),
        _StatTile(icon: Icons.public_rounded, label: 'Länder', value: insights.countryCount.toString()),
        _StatTile(icon: Icons.location_city_rounded, label: 'Städte', value: insights.cityCount.toString()),
        _StatTile(icon: Icons.calendar_month_rounded, label: 'Reisetage', value: insights.travelDays.toString()),
      ],
    );
  }
}
    ...
    return _StatsPanel(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _InfoRow(label: 'Bereiste Länder', value: '${insights.countryCount} von 195'),
          _InfoRow(label: 'Durchschnittliche Reisedauer', value: insights.averageTripDaysLabel),
          _InfoRow(label: 'Dokumentierte Fotos', value: insights.photoTotal.toString()),
        ],
      ),
    );
  }
...
class _RankPanel extends StatelessWidget {
  const _RankPanel({required this.title, required this.emptyText, required this.items});
  ...
  @override
  Widget build(BuildContext context) {
    final maxValue = items.isEmpty ? 1 : items.map((item) => item.value).reduce((a, b) => a > b ? a : b);
      ...
      child: Column(
        ...
        children: [
          ...
          if (items.isEmpty)
            ...
            ...items.take(5).map((item) => _RankRow(item: item, maxValue: maxValue)),
        ],
      ),
    );
  }
    ...
    return _StatsPanel(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          if (items.isEmpty)
            const Text('Noch keine Kontinente aus Reisen berechnet.', style: TextStyle(color: AppColors.textMuted))
          else
            ...items.map(
              (item) => _InfoRow(
                label: item.label,
      ...
      child: Column(
        ...
        children: [
          const _PanelHeader(
            ...
            subtitle: 'Welche Jahre deine Reisegeschichte besonders geprägt haben.',
          ),
          const SizedBox(height: 12),
          if (items.isEmpty)
            const Text('Noch keine Jahresdaten vorhanden.', style: TextStyle(color: AppColors.textMuted))
          else
    ...
    return _StatsPanel(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _InfoRow(label: 'Dokumente gesamt', value: insights.documentCount.toString()),
          _InfoRow(label: 'PDFs', value: insights.pdfCount.toString()),
          _InfoRow(label: 'Bilder / Screenshots', value: insights.imageCount.toString()),
          _InfoRow(label: 'Favorisierte Dokumente', value: insights.favoriteDocumentCount.toString()),
          _InfoRow(label: 'Album-Einträge', value: insights.albumEntryCount.toString()),
          _InfoRow(label: 'Highlights', value: insights.highlightCount.toString()),
          _InfoRow(label: 'Lieblingsmomente', value: insights.favoriteMomentCount.toString()),
        ],
      ),
    );
  }
...
class _TravelInsights {
  ...
  String get worldPercentLabel => '${(worldProgressFraction * 100).toStringAsFixed(1)} %';
  ...
  String get longestTripLabel => longestTrip == null ? 'Noch keine Reise' : '${_tripName(longestTrip!)} · ${longestTrip!.durationDays} Tage';

  String get shortestTripLabel => shortestTrip == null ? 'Noch keine Reise' : '${_tripName(shortestTrip!)} · ${shortestTrip!.durationDays} Tage';

  String get topCountryLabel => topCountries.isEmpty ? 'Noch kein Land' : '${topCountries.first.label} · ${topCountries.first.valueLabel}';

  String get topCityLabel => topCities.isEmpty ? 'Noch keine Stadt' : '${topCities.first.label} · ${topCities.first.valueLabel}';
  ...
  static _TravelInsights fromTrips(List<Trip> trips) {
    ...
    for (final trip in trips) {
      ...
      if (country.isNotEmpty) {
        ...
        normalizedCountries.putIfAbsent(countryKey, () => _GroupedTrips(label: country)).trips.add(trip);
      }
      if (country.isNotEmpty && city.isNotEmpty) {
        final cityKey = '${normalizeGeoName(country)}|${normalizeGeoName(city)}';
        normalizedCities.putIfAbsent(cityKey, () => _GroupedTrips(label: city, subtitle: country)).trips.add(trip);
      }

      final bucket = years.putIfAbsent(trip.startDate.year, () => _YearBucket(trip.startDate.year));
      bucket.trips.add(trip);
    }

    final documents = trips.expand((trip) => trip.documents).toList(growable: false);
    final albumEntries = trips.expand((trip) => trip.albumEntries).toList(growable: false);
    final travelDays = trips.fold<int>(0, (sum, trip) => sum + trip.durationDays);
    final sortedByDuration = trips.toList()..sort((a, b) => b.durationDays.compareTo(a.durationDays));
    ...
    return _TravelInsights(
      ...
      worldProgressFraction: normalizedCountries.isEmpty ? 0 : (normalizedCountries.length / 195).clamp(0, 1).toDouble(),
      ...
      photoTotal: trips.fold<int>(0, (sum, trip) => sum + trip.photoCount) + documents.where(_isImageDocument).length,
      favoriteDocumentCount: documents.where((document) => document.isFavorite).length,
      albumEntryCount: albumEntries.length,
      highlightCount: albumEntries.where((entry) => entry.isHighlight).length,
      favoriteMomentCount: albumEntries.where((entry) => entry.isFavorite).length,
  ...
  static List<_RankItem> _rankCountryItems(Map<String, _GroupedTrips> grouped) {
    final items = grouped.values.map((group) {
      final cities = group.trips.map((trip) => trip.destination.trim()).where((city) => city.isNotEmpty).toSet();
      final days = group.trips.fold<int>(0, (sum, trip) => sum + trip.durationDays);
      return _RankItem(
        label: group.label,
  ...
  static List<_RankItem> _rankContinentItems(Map<String, _GroupedTrips> groupedCountries) {
    final buckets = <String, Set<String>>{};
    for (final group in groupedCountries.values) {
      final continent = continentForCountry(group.label);
      buckets.putIfAbsent(continent, () => <String>{}).add(normalizeGeoName(group.label));
    }
  ...
  static bool _isPdfDocument(TravelDocument document) {
    return document.category.id == DocumentCategories.pdf.id || document.fileExtension.trim().toLowerCase() == 'pdf';
  }
...
class _RankItem {
  const _RankItem({required this.label, required this.subtitle, required this.value, required this.valueLabel});
...
class _YearBucket {
  ...
  _YearStat toStat() {
    final countries = trips.map((trip) => trip.country.trim()).where((country) => country.isNotEmpty).toSet();
    final cities = trips.map((trip) => trip.destination.trim()).where((city) => city.isNotEmpty).toSet();
    return _YearStat(
      year: year,
      tripCount: trips.length,
